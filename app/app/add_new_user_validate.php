...
//                }
        ...
        if ($peselValidator->checkLength($pesel) == true) {
            ...
            if ($birth_date==false){
                $_SESSION['pesel_error'] = '<span style="color:red">Incorrect PESEL given!<br/></span>';
                $validated = false;
            }
            else {
                unset($_SESSION['pesel_error']);
            ...
            $_SESSION['pesel_error'] = '<span style="color:red">PESEL consists of 11 digits!<br/></span>';
            $validated = false;
        }


        if ($r_password!=$password) {
            $_SESSION['p_error'] = '<span style="color:red">Passwords are not equal!<br/></span>';
            $validated = false;
        }
        else {
            unset($_SESSION['p_error']);
            if (strlen($password) < 4 ) {
                $_SESSION['p_error'] = '<span style="color:red">Password is too short!<br/></span>';
                $validated = false;
            }
            else {
                unset($_SESSION['p_error']);
